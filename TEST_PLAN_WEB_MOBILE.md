...
2026-09-06  Claude(chat)  A/logout-visibility  FAIL->FIXED->FAIL->FIXED  Logout worked via each role's More screen but wasn't visible from Dashboard at all — found live by Da. First fix (commit a28a9c6): Log Out icon in shared AppHeader with Alert.alert() confirmation. Re-tested live: icon rendered and was tappable but did NOTHING on web — Alert.alert() is not reliably implemented by react-native-web (no dialog appears). Per owner feedback, moved to Sidebar.tsx's bottom instead with a clearly-labeled "Log out" text (not just an icon), and switched confirmation to Platform.OS-branched window.confirm() on web / Alert.alert() unchanged on native (commit a2b6801). Re-tested live by Da: sidebar "Log out" visible and confirmed working — but window.confirm()'s generic browser-styled popup was flagged as not good enough presentation. Replaced with a new reusable branded ConfirmDialog modal component (commit e6eda95). Not yet re-verified live by Da as of this entry — check next session.
2026-09-06  Claude(chat)  C/teacher-static-audit  PARTIAL  Da asked to confirm Teacher role is "fully operational and without errors" before any live testing of section C had happened (progress log only had section A at this point) — did NOT claim this without evidence. Static audit only: tsc --noEmit clean on both backend/mobile; verified (by reading code, not trusting commit messages) that the materials/topics teacher-scoping fix (commit d93757b) correctly matches the write-side checkTeacherContentScope; confirmed no print/export UI exists anywhere in teacher screens. FOUND a real gap: teacher could read GET /finance/invoices and GET /fee-items (scoped to own school) — contradicted the "no Finance access" expectation in Da's own test scope, unlike the Operations Admin wall which was already deliberately blocked with a clear comment. Confirmed with Da: lock it down. Fixed (commit 4e7e407): renamed blockOpsAdmin->blockNonFinanceRoles, now also blocks role==='teacher' with 403; removed the now-dead teacher SQL branch in GET /invoices. Section C's actual click-through testing (score entry, attendance, weekly efforts, materials list rendering against real data) has NOT happened yet — still needs a real live pass.
